...
houseStatistics.number_of_democrats = democrats.length;

var republicans = getMembers('r');
houseStatistics.number_of_republicans = republicans.length;

var independents = getMembers('i');
houseStatistics.number_of_independents = independents.length;

houseStatistics.number_of_congressmen = allMembers.length;
...
houseStatistics.democrat_vote_pct = pctVotesWithParty(democrats);

houseStatistics.republican_vote_pct = pctVotesWithParty(republicans);

houseStatistics.independent_vote_pct = pctVotesWithParty(independents);
...
houseStatistics.least_loyal = getThe('least', 'loyal', allMembers);
houseStatistics.most_loyal = getThe('most', 'loyal', allMembers);
houseStatistics.least_engaged = getThe('most', 'absent', allMembers);
houseStatistics.most_engaged = getThe('least', 'absent', allMembers);
...
var firstGlanceInfo = [{"party" : "Democrats", "reps" : houseStatistics.number_of_democrats, "votes_pct" : houseStatistics.democrat_vote_pct},
{"party" : "Republicans", "reps" : houseStatistics.number_of_republicans, "votes_pct" : houseStatistics.republican_vote_pct},
{"party" : "Independent", "reps" : houseStatistics.number_of_independents, "votes_pct" : houseStatistics.independent_vote_pct},
{"party" : "Total", "reps" : houseStatistics.number_of_congressmen, "votes_pct" : ((houseStatistics.democrat_vote_pct+houseStatistics.republican_vote_pct+houseStatistics.independent_vote_pct)/3)}];
...
if (document.title.includes('Loyalty')) {
  ...
  houseStatistics.least_loyal.forEach(e => createRows('#leastLoyal', e));
  houseStatistics.most_loyal.forEach(e => createRows('#mostLoyal', e));

} else if (document.title.includes('Attendance')){

  houseStatistics.least_engaged.forEach(e => createRows('#leastEngaged', e));
  houseStatistics.most_engaged.forEach(e => createRows('#mostEngaged', e));
